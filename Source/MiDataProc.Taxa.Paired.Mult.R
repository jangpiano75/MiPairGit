...
volcano_visualize_individual <- function(volcano_dat_fin, species){
  ...
  if(species){
    col_in <<- c("red", "green", "blue", "goldenrod", "magenta", "grey") 
  }else{
    col_in <<- c("red", "green", "blue", "goldenrod", "magenta") 
  }
  fig_list <- list() 
  ...
  fig_list[[1]] <- plot_ly(data = volcano_dat_fin[[1]]) %>% add_trace(data = volcano_dat_fin[[1]][volcano_dat_fin[[1]]$global_sig == "glob: non-sig",], x = ~as.numeric(logFC), y = ~-log10(as.numeric(Adj.P.value) + 0.0001), symbols = "cross", color = ~rank, colors = "Set1", marker = list(size = 7), opacity = 0.7,  type = "scatter",  mode = "markers", text = ~paste("logFC:", as.numeric(logFC), "<br>Adj. P.value:", as.numeric(Adj.P.value) , "<br>", rank, ":", name), hoverinfo = 'text') %>% add_trace(data = volcano_dat_fin[[1]][volcano_dat_fin[[1]]$global_sig == "glob: sig",], x = ~as.numeric(logFC), y = ~-log10(as.numeric(Adj.P.value) + 0.0001), symbol = factor("glob: sig", levels = c("glob: sig", "glob: non-sig")), symbols = 27, color = ~rank, colors = "Set1",  marker = list(size = 7), opacity = 0.7, type = "scatter",  mode = "markers",text = ~paste("logFC:", as.numeric(logFC), "<br>Adj. P.value:", as.numeric(Adj.P.value) , "<br>", rank, ":", name), hoverinfo = 'text', showlegend = FALSE)  %>% layout(xaxis = list(title = title_vec[1], ticktext = list("log<sub>2</sub>FC"), tickvals = list(0)), yaxis = list(title =  list(text = "-log<sub>10</sub>Q")), plot_bgcolor = "#FFFFFF", shapes = list(list(type = "rect", fillcolor = "Blue", line = list(color = "Blue"), opacity = 0, y0 = 1.301029996, y1 = ylim_val, x0 = - x_lim_1 - 1, x1= -2), list(type = "rect", fillcolor = "red", line = list(color = "red"), opacity = 0, y0 = 1.301029996, y1 = ylim_val, x0 = 2, x1 = x_lim_1 + 1)), showgrid = FALSE) 
  ...
  for (i in 2:(length(volcano_dat_fin))){
    ...
    fig_list[[i]] <- plot_ly(volcano_dat_fin[[i]], x = ~as.numeric(logFC), y = ~-log10(as.numeric(Adj.P.value) + 0.0001), symbol = ~ global_sig, symbols = c("cross", 27), color = ~rank, colors = "Set1",  marker = list(size = 7), opacity = 0.7, type = "scatter",  mode = "markers", text = ~paste("logFC:", as.numeric(logFC), "<br>Adj. P.value:", as.numeric(Adj.P.value) , "<br>", rank, ":", name), showlegend = FALSE, hoverinfo = 'text') %>% layout(xaxis = list(title = title_vec[i], ticktext = list("log<sub>2</sub>FC"), tickvals = list(0)), yaxis = list(title =  "-log<sub>10</sub>Q"), plot_bgcolor = "#FFFFFF", shapes = list(list(type = "rect", fillcolor = "Blue",line = list(color = "Blue"), opacity = 0, y0 = 1.301029996, y1 = ylim_val,  x0 = -x_lim-1, x1= -2), list(type = "rect", fillcolor = "red", line = list(color = "red"), opacity = 0, y0 = 1.301029996, y1 = ylim_val, x0 = 2, x1 = x_lim + 1)))
  }
